...
def createtable(request):
    if request.method == 'POST':
        ...
        if len(name) < 2 or len(faculty) < 2 or len(address1) < 2:
            ...
            table = Table(name=name, faculty=faculty, semester=semester, address1=address1,
                          address2=address2, batch=batch, email=email)
            table.save()
            messages.success(
                request, "Your message has been successfully sent")
    all_fac = models.Faculty.objects.all()
    context = {'faculty': all_fac}
